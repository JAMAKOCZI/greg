...
import { normalizeEffort, normalizeModelId } from "./models.mjs";
...
export function normalizeSettings(raw) {
  ...
  // Empty / missing → product defaults (single model + high effort)
  const model = normalizeModelId(src.model) || DEFAULT_SETTINGS.model;
  const effortRaw = Object.prototype.hasOwnProperty.call(src, "effort")
    ? src.effort
    : src.reasoningEffort;
  const effort = normalizeEffort(effortRaw) || DEFAULT_SETTINGS.effort;
  const defaultCwd =
...
export function mergeSettings(current, patch) {
  ...
  if (Object.prototype.hasOwnProperty.call(patch, "model")) {
    const m = patch.model;
    // Empty clears back to product default model
    next.model =
      m == null || m === ""
        ? DEFAULT_SETTINGS.model
        : typeof m === "string" && m.trim()
          ? normalizeModelId(m) || DEFAULT_SETTINGS.model
          : current.model;
  }
  if (
    ...
    const e = Object.prototype.hasOwnProperty.call(patch, "effort")
      ? patch.effort
      : patch.reasoningEffort;
    next.effort =
      e == null || e === ""
        ? DEFAULT_SETTINGS.effort
        : normalizeEffort(e) || DEFAULT_SETTINGS.effort;
  }
