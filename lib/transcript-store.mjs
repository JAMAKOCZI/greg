...
import { mergeToolMeta } from "./transcript-messages.mjs";
...
export class TranscriptStore {
  ...
  async upsertToolMessage(id, message) {
    return this.#withLock(id, async () => {
      ...
      if (toolCallId) {
        ...
        if (idx >= 0) {
          doc.messages[idx] = {
            ...
            ts,
            text,
            meta: mergeToolMeta(doc.messages[idx].meta, message.meta),
          };
          doc.updatedAt = Date.now();
          await this.#atomicWrite(doc);
          return doc;
      ...
      doc.messages.push({
        ...
        text,
        ...(message.meta
          ? { meta: mergeToolMeta(null, message.meta) }
          : {}),
      });
      doc.updatedAt = Date.now();
      await this.#atomicWrite(doc);
   ...
   * @returns {Promise<Transcript|null>}
   */
  async upsertPlanMessage(id, text, opts = {}) {
    return this.#withLock(id, async () => {
      const doc = await this.load(id);
      if (!doc) return null;
...
function normalizeDoc(doc) {
  ...
  if (doc.source && typeof doc.source === "object") {
    ...
    if (kind === "grok" && doc.source.sessionId) {
      out.source = {
        kind: "grok",
        sessionId: String(doc.source.sessionId),
        ...(typeof doc.source.path === "string"
          ? { path: doc.source.path }
          : {}),
        ...(Number(doc.source.importedAt)
          ? { importedAt: Number(doc.source.importedAt) }
          : {}),
      };
    }
  }
...
function toSummary(doc) {
  return {
    ...
    ...(doc.source?.kind === "grok"
      ? { source: "grok", sourceSessionId: doc.source.sessionId }
      : {}),
  };
}
