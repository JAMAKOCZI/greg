...
/** @typedef {"user"|"agent"|"system"|"tool"|"plan"|"permission"} MessageRole */
...
export class TranscriptStore {
  ...
  /**
   * Create a new empty transcript (or replace if id provided and exists — caller choice).
  ...
  async #atomicWrite(doc) {
    ...
    const tmp = join(
      this.rootDir,
      `.${sanitizeId(doc.id)}.${process.pid}.${Date.now()}.tmp`,
    );
