 ...
 * Returns only directories (plus `..` via `parent`). Localhost auth is the trust boundary.
 *
...
export async function listDirectories(pathInput, opts = {}) {
  ...
  // Do not allow using filesystem root as a browse root for the picker either
  if (abs === "/" || abs === "\\") {
    return {
      ok: false,
      error: "Cannot browse filesystem root (/) — pick a home or project folder",
      code: "ROOT_FORBIDDEN",
    };
  }
  ...
  if (abs === "/" || abs === "\\") {
    return {
      ok: false,
      error: "Cannot browse filesystem root (/)",
      code: "ROOT_FORBIDDEN",
    };
  }
  ...
  for (const name of names) {
    ...
    try {
      ...
      // Skip dirs that resolve to filesystem root only
  ...
  const parent =
    parentRaw && parentRaw !== abs && parentRaw !== "/" && parentRaw !== "\\"
      ? parentRaw
      : null;
