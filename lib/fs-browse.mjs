...
/**
 * Disk roots for the folder picker drive select (no Home / placeholder entries).
 * First entry is the system drive and should be the UI default.
 * @returns {Promise<{
 *   ok: true,
 *   platform: string,
 *   home: string,
 *   defaultRoot: string,
 *   roots: { id: string, path: string, label: string, system?: boolean }[],
 * }>}
 */
export async function listFilesystemRoots() {
  ...
  if (plat === "win32") {
    // Prefer C: as system drive; then other letters
    ...
    // If C: missing, mark the first available as system
    if (roots.length && !roots.some((r) => r.system)) {
      roots[0].system = true;
    }
  } else {
    // System volume is /
    roots.push({ id: "root", path: "/", label: "/", system: true });
  ...
  const defaultRoot =
    roots.find((r) => r.system)?.path || roots[0]?.path || home;

  return { ok: true, platform: plat, home, defaultRoot, roots };
}
