...
export class TabRegistry {
  ...
  /**
   * Register a new tab. Refuses to overwrite an existing id (avoids leaking
   * the previous bridge/SSE). Callers that intentionally replace must
   * `delete` (after stop/endSse) first.
   *
   * @param {string} tabId
   * @param {{
  ...
  create(tabId, opts) {
    if (this._tabs.has(tabId)) {
      throw new Error(
        `TabRegistry.create: tab "${tabId}" already exists — delete it first`,
      );
    }
    const now = Date.now();
    const title =
  ...
  setTitle(tabId, title, now = Date.now()) {
    const entry = this._tabs.get(tabId);
    if (!entry) return undefined;
    entry.title = typeof title === "string" ? title.trim() || null : null;
    entry.lastActiveAt = now;
    return entry;
  }
